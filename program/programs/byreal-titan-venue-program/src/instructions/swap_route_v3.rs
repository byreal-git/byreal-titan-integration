...
use crate::instructions::venues::byreal_clmm;
...
impl SwapRouteV3<'_> {
    pub fn execute<'info>(
        ...
        require!(
            ...
            RouteError::InvalidSwapInput
        );
        require!(swaps.len() <= MAX_SWAPS, RouteError::InvalidSwapInput);

        let mints_count = mints as usize;
        require!(mints_count <= MAX_MINTS, RouteError::InvalidSwapInput);
        require!(
            ctx.remaining_accounts.len() >= 2 * mints_count,
            RouteError::MissingRemainingAccount
        );
        require!(swaps[0].from == 0, RouteError::InvalidSwapInput);
        ...
        let input_mint = ctx
            .remaining_accounts
            .get(mints_count)
            .ok_or(RouteError::MissingRemainingAccount)?;
        ...
        let effective_amount = match ledger_amount {
            Some(stored) => {
                ...
                let delta = current
                    .checked_sub(stored)
                    .ok_or(RouteError::InvalidAccountData)?;
                require!(delta > 0, RouteError::InvalidAccountData);
                delta
            }
            None => amount,
        };
        ...
        for swap in swaps.iter() {
            ...
            require!(account_increment > 0, RouteError::InvalidSwapInput);
            require!(
        ...
        for (i, account) in ctx.remaining_accounts.iter().enumerate() {
            if titan_pda_ata_bitmap.is_set(i) {
                ...
                require!(
                    ...
                    RouteError::ReservedBalanceViolation
                );
                balance_idx += 1;
            }
        }
...
fn perform_cpi_swap<'info>(
    ...
    let venue_program_id = *account_infos
        .last()
        .ok_or(RouteError::MissingRemainingAccount)?
        .key;
    let instructions = match swap.venue {
        Venue::ByrealClmm { .. } => {
            byreal_clmm::swap_v3_dyn(venue_program_id, amount, account_metas)?
        }
    };
...
fn read_token_ledger<'info>(
    ...
    if data.len() < 48 {
        return err!(RouteError::InvalidAccountData);
    }

    let token_account_slice = data.get(8..40).ok_or(RouteError::InvalidAccountData)?;
    let token_account = Pubkey::new_from_array(
        token_account_slice
            .try_into()
            .map_err(|_| RouteError::InvalidAccountData)?,
    );
    require!(
    ...
    let amount_slice = data.get(40..48).ok_or(RouteError::InvalidAccountData)?;
...
fn transfer_tokens<'info>(
    ...
    if token_program.key().eq(&TOKEN_PROGRAM_2022_ID) {
        let mint = mint.ok_or(RouteError::MissingRemainingAccount)?;
...
fn extract_token_amount(account: &AccountInfo) -> Result<u64> {
    ...
    let slice = data.get(64..72).ok_or(RouteError::InvalidAccountData)?;
...
fn extract_token_owner(account: &AccountInfo) -> Result<Pubkey> {
    let data = account.try_borrow_data()?;
    let slice = data.get(32..64).ok_or(RouteError::InvalidAccountData)?;
...
fn extract_mint_decimals(mint: &AccountInfo) -> Result<u8> {
    mint.try_borrow_data()?
        .get(44)
        .copied()
        .ok_or(RouteError::InvalidAccountData.into())
}
